...
{
    // g_pixelScaleY is target texels per emulated pixel, so the radius is
    // read as a distance down the PICTURE rather than down the target.
    float  ty   = 1.0 / max (g_outputH, 1.0);
    float  step = ty * max (g_pixelScaleY, 0.001) * max (g_bloomRadius, 0.001);
