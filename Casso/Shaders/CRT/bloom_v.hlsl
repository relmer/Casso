...
// Casso modifications: vertical leg of the separable Gaussian. Pair with
//   bloom_h.hlsl. Both omit the luminance threshold pass for simplicity (see
//   bloom_h header).
...
{
    float  texelY = 1.0 / max (g_outputH, 1.0);
    float  step   = texelY * max (g_bloomRadius, 0.001);
