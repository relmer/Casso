...
//  (GetScreenRect) the viewport composites into, and Paint draws the housing in
...
public:
    ...
    float  GetSceneScale () const { return m_hidden ? 1.0f : m_sceneScale; }
    ...
    static SIZE  GetCenterSizeForScreenPx (int screenWpx, int screenHpx);

    // Collapse the frame: no housing, and GetScreenRect() falls back to the
...
private:
    ...
    static float     GetEdgeInset (float y, float top, float bottom,
