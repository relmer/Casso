...
void ListView::SetRect (const RECT & rect)
{
...
void ListView::SetRows (std::vector<std::vector<Cell>> rows)
{
    bool  wasSticky = m_stickyTail;
    int   maxTop    = 0;
...
void ListView::SetColumnVisible (size_t idx, bool visible)
{
    HRESULT  hr = S_OK;
...
//  fall back to widthDp / measured / stretch. The widget enforces a
//  minimum of 1px.
//
////////////////////////////////////////////////////////////////////////////////

void ListView::SetColumnOverrideWidthPx (size_t idx, int px)
{
    HRESULT hr = S_OK;
...
//  sentinel tells ComputeColumnLayout to fall back to widthDp /
//  measured / stretch.
//
////////////////////////////////////////////////////////////////////////////////

int ListView::GetColumnOverrideWidthPx (size_t idx) const
{
    HRESULT  hr     = S_OK;
    int      result = -1;
...
int ListView::GetColumnEffectiveWidthPx (size_t idx) const
{
    HRESULT           hr      = S_OK;
    int               width   = -1;
    std::vector<int>  xs;
...
int ListView::GetTotalMeasuredWidthPx () const
{
    int  sum = 0;
...
void ListView::MeasureColumnsPx (DwriteTextRenderer & text)
{
    HRESULT  hr      = S_OK;
...
int ListView::GetVisibleColumnCount () const
{
...
int ListView::GetNthVisibleColumnIndex (int n) const
{
    int  result = -1;
    int  seen   = 0;
...
int ListView::GetVisibleIndexOfColumn (size_t absCol) const
{
    HRESULT  hr     = S_OK;
    int      result = -1;
    int      seen   = 0;
...
void ListView::SetSelectedRow (int r)
{
    HRESULT  hr   = S_OK;
...
int ListView::GetVisibleRowCapacity () const
{
    HRESULT  hr      = S_OK;
    int      result  = 0;
...
int ListView::GetMaxTopRow () const
{
...
void ListView::SetTopRow (int topRow)
{
...
void ListView::ScrollByWheelDelta (int wheelDelta, int linesPerNotch)
{
    HRESULT  hr      = S_OK;
    int      notches = 0;
...
ListView::ScrollbarMetrics ListView::GetScrollbarGeometry () const
{
    HRESULT           hr      = S_OK;
    ScrollbarMetrics  m;
...
bool ListView::HitTestScrollbarThumb (int xPx, int yPx) const
{
    HRESULT           hr     = S_OK;
    bool              result = false;
...
bool ListView::HitTestScrollbarTrack (int xPx, int yPx) const
{
    HRESULT           hr     = S_OK;
    bool              result = false;
    ScrollbarMetrics  m      = GetScrollbarGeometry();
...
bool ListView::HitTestScrollbarArrowUp (int xPx, int yPx) const
{
    HRESULT           hr     = S_OK;
    bool              result = false;
    ScrollbarMetrics  m      = GetScrollbarGeometry();
...
bool ListView::HitTestScrollbarArrowDown (int xPx, int yPx) const
{
    HRESULT           hr     = S_OK;
    bool              result = false;
    ScrollbarMetrics  m      = GetScrollbarGeometry();
...
void ListView::PageFromTrackClick (int yPx)
{
    HRESULT           hr  = S_OK;
    ScrollbarMetrics  m   = GetScrollbarGeometry();
    int               cap = GetVisibleRowCapacity();
...
void ListView::BeginThumbDrag (int grabYPx)
{
    ScrollbarMetrics  m = GetScrollbarGeometry();
...
void ListView::UpdateThumbDrag (int yPx)
{
    HRESULT           hr       = S_OK;
    ScrollbarMetrics  m        = GetScrollbarGeometry();
    int               maxTop   = GetMaxTopRow();
...
int ListView::GetRequiredRowsForHeightPx (int heightPx) const
{
    HRESULT  hr      = S_OK;
    int      result  = 0;
    int      rowH    = m_scaler.Px (s_kRowHeightDp);
...
int ListView::GetRequiredHeightPx () const
{
    int  rows    = (int) m_rows.size();
    int  rowH    = m_scaler.Px (s_kRowHeightDp);
    int  headerH = m_showHeader ? m_scaler.Px (s_kHeaderHeightDp) : 0;
...
int ListView::HitTestColumnResize (int xPx, int yPx, int tolerancePx) const
{
    HRESULT  hr          = S_OK;
    int      result      = -1;
    int      headerH     = m_showHeader ? m_scaler.Px (s_kHeaderHeightDp) : 0;
...
int ListView::HitTestHeaderColumn (int xPx, int yPx) const
{
    HRESULT          hr      = S_OK;
    int              result  = -1;
    int              headerH = m_showHeader ? m_scaler.Px (s_kHeaderHeightDp) : 0;
...
int ListView::HitTestRow (int xPx, int yPx) const
{
    HRESULT  hr      = S_OK;
    int      result  = -1;
    int      rowH    = m_scaler.Px (s_kRowHeightDp);
...
void ListView::Paint (DxUiPainter & painter, DwriteTextRenderer & text) const
{
    HRESULT          hr         = S_OK;
...
ListView::Palette ListView::MakePalette () const
{
    Palette  pal = {};
...
{
    HRESULT  hr        = S_OK;
    float    headerH   = (float) m_scaler.Px (s_kHeaderHeightDp);
...
void ListView::PaintHeaderFocusMarkers (
    DxUiPainter            & painter,
    const Palette          & pal,
    float                    x,
    float                    y,
...
//  width. Fixed/auto columns take their override / widthDp / measured
//  width; the first stretch column absorbs whatever space remains.
//
////////////////////////////////////////////////////////////////////////////////

void ListView::ComputeColumnLayout (float fullW, std::vector<int> & xs, std::vector<int> & ws) const
{
    int  fixedTotal = 0;
    int  stretchIdx = -1;
    int  x          = 0;
...
void ListView::PaintScrollArrow (DxUiPainter & painter,
                                 float          ax,
                                 float          ay,
                                 float          aw,
                                 float          ah,
