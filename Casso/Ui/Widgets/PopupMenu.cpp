...
namespace
{
    constexpr int    s_kItemHeightDp    = 26;
    constexpr int    s_kItemPadLeftDp   = 28;
    constexpr int    s_kItemPadRightDp  = 16;
    constexpr int    s_kCheckGlyphOffDp = 10;
    ...
    const RECT          & hostClient)
{
    ...
    int      itemH     = m_scaler.Px (s_kItemHeightDp);
    int      padL      = m_scaler.Px (s_kItemPadLeftDp);
    int      padR      = m_scaler.Px (s_kItemPadRightDp);
    int      border    = m_scaler.Px (s_kBorderDp);
    int      minW      = m_scaler.Px (s_kMinWidthDp);
    int      widestPx  = 0;
    int      width     = 0;
    int      height    = 0;
    int      left      = anchorX;
...
bool PopupMenu::HitTest (int x, int y) const
{
    return m_visible
...
int PopupMenu::HitTestIndex (int x, int y) const
{
    int  border = m_scaler.Px (s_kBorderDp);
    int  itemH  = m_scaler.Px (s_kItemHeightDp);
...
bool PopupMenu::OnLButtonDown (int x, int y)
{
...
bool PopupMenu::OnLButtonUp (int x, int y)
{
...
bool PopupMenu::OnKey (WPARAM vk)
{
    if (!m_visible) { return false; }
...
void PopupMenu::Paint (DxUiPainter & painter, DwriteTextRenderer & text) const
{
    ...
    int       padL     = m_scaler.Px (s_kItemPadLeftDp);
    int       padR     = m_scaler.Px (s_kItemPadRightDp);
    int       glyphX   = m_scaler.Px (s_kCheckGlyphOffDp);
    float     fontDip  = (float) m_scaler.Pxf (s_kFontDp);
    uint32_t  bgArgb   = 0;
    uint32_t  bgHover  = 0;
    uint32_t  fgArgb   = 0;
