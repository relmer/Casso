...
bool Toggle::HitTest (int x, int y) const
{
    if (!m_enabled)
    {
        return false;
...
void Toggle::SetMouseHover (int x, int y)
{
    m_hover = HitTest (x, y);
    if (!m_hover)
    {
...
bool Toggle::OnLButtonDown (int x, int y)
{
    if (!HitTest (x, y))
    {
        return false;
...
bool Toggle::OnLButtonUp (int x, int y)
{
...
void Toggle::Paint (DxUiPainter & painter, DwriteTextRenderer & text) const
{
    ...
    constexpr float     s_kLabelGapDp   = 8.0f;
    constexpr float     s_kFontDip      = 13.0f;

    HRESULT  hr         = S_OK;
    float    pillW      = m_scaler.Pxf (s_kPillWidthDp);
    float    pillH      = m_scaler.Pxf (s_kPillHeightDp);
    float    thumbInset = m_scaler.Pxf (s_kThumbInsetDp);
