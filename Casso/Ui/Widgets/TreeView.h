...
struct TreeNode
{
    ...
    TreeCapabilityFlag            capabilityFlag = TreeCapabilityFlag::Optional;
    bool                          checked        = false;
    bool                          expanded       = true;
...
public:
    ...
    void  SetNodes     (std::vector<TreeNode> nodes) { m_nodes = std::move (nodes); RebuildFlatRows(); }
    ...
    const TreeNode *  NodeAt    (int flatIndex) const;
    TreeNode       *  NodeAtMutable (int flatIndex);
...
private:
    ...
    void  FlattenRecursive (const TreeNode & node, std::vector<int> & path, int depth);
