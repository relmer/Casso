...
//  the caller bumps its drop counter.
//
...
bool InputEventRing::TryPush (const InputEvent & e) noexcept
{
...
//  slot. The slot is copied out before the release-store of the new head
//  so the producer can safely overwrite it on its next push.
//
////////////////////////////////////////////////////////////////////////////////
...
//  Drain
//
//  UI thread only. Pops up to `maxCount` entries in FIFO order. Folds the
//  per-entry head update into a single release-store at the end so the
//  drain is one acquire + N relaxed copies + one release instead of N
//  acquire/release pairs.
//
////////////////////////////////////////////////////////////////////////////////

uint32_t InputEventRing::Drain (InputEvent * out, uint32_t maxCount) noexcept
{
    uint32_t  head     = m_head.load (std::memory_order_relaxed);
    uint32_t  tail     = m_tail.load (std::memory_order_acquire);
    uint32_t  inFlight = tail - head;
