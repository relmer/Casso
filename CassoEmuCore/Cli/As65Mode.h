...
//  progress, the "N lines assembled" line, and the two further artifacts
//  (`-t` and `-g`).
//
...
private:
    //  The symbol table and the debug file for ONE output, under a name the
    //  caller has already resolved. Split out so a source that produces several
    //  gets a set for each, on the same rule the object and the listing follow.
