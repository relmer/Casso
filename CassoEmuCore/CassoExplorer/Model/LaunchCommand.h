...
public:
    ...
    static constexpr const wchar_t *  kCassoExplorerExe = L"CassoExplorer.exe";
    static constexpr const wchar_t *  kCassoExe   = L"Casso.exe";

    //  `--owner <decimal window>`, and `--title <prefix>` when there is one.
    static std::wstring  MakeCassoExplorerArguments (HWND owner, const std::wstring & titlePrefix);
    ...
    //  without trying.
    static Outcome  LaunchCassoExplorer (IProcessLauncher    & launcher,
