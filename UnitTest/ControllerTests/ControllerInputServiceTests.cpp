...
namespace ControllerTests
{
    ...
    public:
        ...
        TEST_METHOD (AbsentXbox_TheSlotRecheckFindsItWithoutANotification)
        {
            FakeControllerBackend   backend;
            GamePortInputMixer      mixer;
            ControllerInputService  service (backend, mixer);
            ...
            service.SetClock ([&nowMs] () { return nowMs; });
            backend.AddDevice (xbox, true);
            backend.AddDevice (stick);
            service.SetSelection (xbox.unit);
            service.Tick();
            ...
            service.OnDevicesChanged();
            service.Tick();
            Assert::IsTrue (service.GetSnapshot().standIn.has_value(), L"the stick stands in");

            // It comes back over Bluetooth after the rescans that followed any
            // notification, so no device-change event is raised for it.
            backend.AddDevice (xbox, true);

            nowMs += 1000;
            service.Tick();
            Assert::IsFalse (service.GetSnapshot().isSelectedConnected,
                L"the slots are not rechecked more often than every two seconds");

            nowMs += 1000;
            service.Tick();

            Assert::IsTrue  (service.GetSnapshot().isSelectedConnected, L"the recheck found it");
            Assert::IsFalse (service.GetSnapshot().standIn.has_value(),  L"and the stand-in gives the axes back");
        }


        TEST_METHOD (Recheck_NotPerformedWhileTheChosenControllerIsAttached)
        {
            FakeControllerBackend   backend;
            GamePortInputMixer      mixer;
            ControllerInputService  service (backend, mixer);
            ...
            // An idle Casso stays idle: the slots are polled only while the
            // controller the user chose is an absent Xbox controller.
            Assert::AreEqual (0, backend.recheckCount);
            Assert::AreEqual (ControllerInputService::kPollPeriodMs, wait.timeoutMs.value(),
                L"the wait is the connected controller's own poll period, untouched");
        }


        TEST_METHOD (Recheck_NotPerformedForAnAbsentDirectInputSelection)
        {
            FakeControllerBackend   backend;
            GamePortInputMixer      mixer;
            ControllerInputService  service (backend, mixer);
            ...
            // DirectInput arrivals raise their notification reliably (R4), so
            // only an absent Xbox controller has its slots polled.
            Assert::AreEqual (0, backend.recheckCount);
        }


        TEST_METHOD (AbsentXbox_TheThreadWakesForTheNextRecheck)
        {
            FakeControllerBackend   backend;
            GamePortInputMixer      mixer;
            ControllerInputService  service (backend, mixer);
            ...
            // Nothing is attached, so there are no events and no poll period.
            // Without the deadline the thread would never wake to recheck.
            Assert::IsTrue (wait.timeoutMs.has_value(), L"the wait has a deadline");
            Assert::IsTrue (wait.timeoutMs.value() <= ControllerInputService::kXInputRecheckMs);

            nowMs += 1500;
            wait   = service.Tick();

            Assert::IsTrue (wait.timeoutMs.has_value() && wait.timeoutMs.value() <= 500,
                L"and it counts down to the next recheck rather than restarting");
        }
    };
}
