...
- [x] T080 [US6] Run the per-phase gate and commit with the measurement
      - style tree sweep 1,410 files OK; Debug x64 5096/5096; Release x64 5094/5094;
...
- [x] T082 [US7] Add golden-image storage and a pixel-exact comparison helper with no tolerance parameter (FR-008a)
...
- [x] T087 [P] [US7] Test: a synthetic framebuffer through the full pass chain matches its checked-in golden pixel-exact
- [x] T088 [P] [US7] Test: changing one CRT parameter changes the image in the documented direction
- [x] T089 [P] [US7] Test: a mixed span of sources at known gains and pans matches expected samples, including at the clipping boundary
...
- [x] T133 [US9] Move the `apple2e-family-mmu` special case (`MachineManager.cpp:266`) and `WireApple2cRomBank` (`:961`) into the machines that own them; the //e's MMU and the //c's ROM bank are wired from the executable today
- [x] T134 [US9] Replace the 14 `IsApple2c()` call sites with capability queries on the machine — which drive mesh, whether a switch band exists, whether a switch bar is shown — since every one of them is a presentation question, not an emulation one
...
## Phase 15: Retiring the test tree's second machine, and per-machine coverage

**Goal**: one machine builder, and each machine tested for what it declares.

The suite carried its own //e, //c, ][ and ][+ builders because the production
one was a method on a class no test could construct. That is no longer true,
