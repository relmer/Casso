...
`DxuiCommand`, one declaration per action; `DxuiPopupMenu`, one dropdown
...
│   └── DxuiCommand.h                  # NEW
├── Widgets/
│   ├── DxuiPopupMenu.h/.cpp           # EXTENDED: command items + menu bar painting
│   ├── DxuiContextMenu.h/.cpp         # NEW, one static Show
│   ├── DxuiToolbar.h/.cpp             # NEW, from CommandToolbar
│   └── DxuiMenuBar.h/.cpp             # REBUILT over DxuiPopupMenu; subitem type removed
└── Dxui.vcxproj
...
│   └── DxuiWidgetIDxuiControlTests.cpp # DxuiPopupMenu rows become DxuiPopupMenu + DxuiToolbar rows
├── UiTests/
...
2. **DxuiPopupMenu** built from `DxuiPopupMenu` plus the menu bar's item
   painting, with tests. `DxuiPopupMenu` still exists.
3. **DxuiMenuBar over DxuiPopupMenu.** `DxuiMenuBarSubitem` becomes
   `DxuiPopupMenuItem` over `DxuiCommand`. `MainMenu` adapts its `s_kEntries`
...
5. **DxuiToolbar** extracted from `CommandToolbar`, pickers on
   `DxuiPopupMenu`, with tests. `CommandToolbar` becomes a temporary thin
...
return. Owns one `DxuiPopupMenu`. `Open` shows it under the title with that
