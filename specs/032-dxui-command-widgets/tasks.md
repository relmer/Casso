...
- [ ] T032 [US4] In `DxuiToolbar.cpp`, replace the three `DxuiPopupMenu` members with one `DxuiPopupMenu` plus a per-picker item list, `openedOn`, preview and commit sinks keyed by command id; preview on highlight change, commit once on select, replay `openedOn` on uncommitted close
