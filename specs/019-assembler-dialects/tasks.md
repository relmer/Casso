...
- [ ] T045a [P] [US1] **⚠️ RESOLVE THE LICENSING CONFLICT FIRST.** This task says to bulk-capture the ~40 vendor source files on the disk as corpus entries, but the spec's Assumptions say "only source authored here and the bytes it produced are committed" — and vendor source is Glen Bredon's copyrighted work, not ours. Committing `CLOCK.S`, `PI.*.S` and the rest verbatim would put third-party source in the repository, which is what the disk-image exclusion exists to prevent. Options, none yet chosen: use vendor source as an uncommitted local smoke test whose *result* is recorded but whose text is not; commit only a hash of source plus its captured bytes; or author equivalent source here exercising the same constructs. Until settled, capture vendor source **locally for validation only** and commit nothing from it
