...
- [ ] T021d [US1] **Settle on entry one**: does Merlin's editor store pasted source byte-for-byte, or normalize it? Record the answer in `specs/019-assembler-dialects/research.md` before the harness shape depends on it
- [ ] T021e [US1] Record in `UnitTest/MerlinCorpus/README.md` the residual gap automation cannot close: if the paste garbled *and* Merlin assembled the garbled source, the entry is self-consistent and simply tests a construct nobody intended. The `discriminates` flag catches the worst version — a garble that destroys the Merlin construct stops the entry failing under AS65 — but not one that merely changes an operand. Read the first few entries by eye; nothing downstream will report it
...
- [ ] T045a [P] [US1] Bulk-capture the **remaining vendor source on the disk** — roughly forty files — as corpus entries in `UnitTest/MerlinCorpus/CorpusEntries.h`. Assemble and compare; do **not** read them for spec revisions. A byte mismatch is specific, attached to a test, and cannot be forgotten, whereas a reading costs attention now and buys no regression coverage
