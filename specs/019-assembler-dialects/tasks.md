...
- [ ] T025d [P] [US1] Capture a macro-local label entry into `UnitTest/MerlinCorpus/CorpusEntries.h`: expand two macros that each define the same bare label, as the vendor library does three times with `NC`, to settle how Merlin scopes labels inside macro bodies
