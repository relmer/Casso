...
- [ ] T052 [US2] Set `AssemblerOptions::dialect` from `CommandLineOptions::dialect` in `CassoCli/CommandLine.cpp`. There are now **two** routes to AS65 and they must stay distinguishable: `as65` stated (T049a) and the fallback inferring it. The resulting dialect is identical; only the provenance differs, and the reporting table in [contracts/cli.md](./contracts/cli.md) keys off exactly that — stated is reported nowhere, inferred is reported under `-v` or in the listing header. Carry the provenance, not just the dialect
...
- [ ] T071a [P] Add to `CHANGELOG.md` the one behavior change an existing invocation can notice: `CassoCli as65` with no operand stops resolving `as65` as a source filename and becomes a usage error (T049a). Separate from T071 because that entry is about what was added and this one is about what moved — a reader scanning for breakage should not have to find it inside a feature announcement
