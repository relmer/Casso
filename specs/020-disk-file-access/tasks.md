...
- [ ] T002 [P] Define `SectorDecodeReport` and `TrackDecodeOutcome` (`Complete` / `Unformatted` / `Partial`) with per-track 16-bit `coverage`, `duplicated`, `hasDataLoss`, `unrecoveredCount` in `CassoEmuCore/Devices/Disk/SectorDecodeReport.h` per data-model.md. **Track layer, not `VolumeTypes.h`** — these describe nibble decoding, and putting them in the filesystem-layer header would make `NibblizationLayer` include a header from the layer above it
...
- [ ] T016 [P] [US3] Implement `AppleTextCodec` in `CassoCore/AppleTextCodec.h/.cpp`: high-ASCII ↔ host text with line-ending normalization both directions (FR-021); round-trip tests in `UnitTest/AppleTextCodecTests.cpp`
...
- [ ] T032 [US2] Add `put` and `delete` verbs to `ParseDiskOptions` and `DiskCommandRunner` (`--as`, `--type`, `--addr`, `--text`/`--basic`/`--binary`), with write-protect and locked-file refusals reported in intelligible terms rather than raw platform codes (FR-014). Use `--binary`, **not** `--raw`: `--raw` already exists on the assembler as an output shape, and reusing the spelling for an encoding selector in the same parser gives one flag two meanings — worse while spec 019 is editing these files concurrently
