...
because crash safety cannot be unit-tested, and it needed a debug-only
interruption switch inside `Win32DiskFileIo` before it could be done at all —
killing the process from outside never once reached the window, over seventy
attempts. The original image came out byte-identical and bootable after a hard
abort in the pre-replace window; the **temporary survives, and no later run
reclaims it**, so read the task's "no temporary remains" as not satisfied. See
T035 for the measurements and for why that is a real if minor defect rather than
an expected cost. Phase 4's write work is the work here where
...
tests: it is a manual pass over the real file edge, and the debug-only switch
it needed lives in `CassoCli`, which the test assembly does not link — so the
counts are unchanged at 3306 / 3303.
