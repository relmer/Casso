...
`boot` command, and a real-CPU gate that boots each image and **types nothing at
...
now-pointless `UnbuiltCommand_ReportsFailureRatherThanDoingNothingQuietly`.)
...
fourth time. Renaming the `rm` alias to `del` in `s_kDiskCommands` left all nine
help-text tests green, because the sweep asked whether the help *contains* each
command name and `delete` contains `del`, so the one assertion whose entire
purpose is "a command the grammar accepts that the help never mentions" could not
see it. Run unfiltered it is CAUGHT, by `Disk_TerseAliasesResolveToTheDescriptive
Commands`, which has covered the alias since T032; the filtered run is what made
...
same sweep were measured and left for the owner rather than decided, per-command
...
verbatim. `CommandLineParser::GetAllDiskCommands` was added for the same reason
`GetAllSubcommands` exists: the sweep has to run over the grammar's own table,
because a command added there and left out of the help is a capability nobody can
find.
...
- **Direct boot is NOT reachable from the command line, and no task in this phase
  ...
  Phase 6's three tasks are the mechanism and its gates, and `s_kDiskCommands` has no
  command for this. `DirectBootBuilder` is therefore complete, tested and unreachable
  by a user, and there is **no CHANGELOG entry** for it because nothing a user can
  type changed. Whoever adds the command should note that `Build` already produces the
...
- [x] T019 [US3] Implement `DiskCommandRunner` in `CassoEmuCore/Devices/Disk/DiskCommandRunner.h/.cpp`: takes `CommandLineOptions` and an `IDiskFileIo &`, and returns a result carrying the exit status, the stdout payload, and the diagnostic text. It owns every **decision**, command dispatch, exit-status mapping (`0` clean, `1` succeeded-with-complaints, `2` produced no output, matching `as65` and `run`), and failure-message construction naming image, file, and reason (FR-031, FR-033). Covers spec US3 acceptance 4. Tests in `UnitTest/EmuTests/DiskCommandRunnerTests.cpp` against the fake seam
...
- [x] T038 [US4] Add the `boot` command to `ParseDiskOptions` and the runner (FR-024); refuse a program not present on the volume, naming the missing file (FR-025). **The grammar was already complete**, as it was for T032: T017 landed `boot` in `s_kDiskCommands` and the usage block in `CassoCli/CommandLine.cpp` already documents `disk boot <image> <path>`, so this task added the runner arm, its refusals and its tests, and changed no parser row. **Two decisions.** (1) **A binary named as a DOS 3.3 greeting is a COMPLAINT, not a refusal.** Measured with a real processor on the stock master: the disk boots and the program is never executed, because the command a booting DOS 3.3 issues is RUN, which `Dos33FileWriter::WriteHello`'s own comment has said since spec 017. Refusing would be wrong, since a disk whose boot command has been patched by hand is a real thing and its owner knows what they did, so the name is set, the status is 1, and the message says what DOS will do with it. (2) **`UnbuiltCommand_ReportsFailureRatherThanDoingNothingQuietly` is DELETED rather than moved again.** It had migrated from command to command as each was built; `boot` was the last one, so pointing it anywhere now leaves it green for a reason unrelated to its name. `UnknownCommand_SuggestsTheOnesThatExist` covers the property it existed for
...
- [x] T052 Command aliases (`dir`, `cat`, `catalog` → `list`; `del` → `delete`; `read` → `get`; `write` → `put`), following the pattern `ls` and `rm` already use. **`cat` is a reversal of a recorded decision** and the reasoning is worth keeping: it was left out because it collides with the Unix meaning of printing a file's contents, which weighed a convention from another platform above the literal command of the machine this tool exists to serve. On an Apple II, `CAT` lists the disk, and somebody who used one types it first. `catalog` is there for the same reason; `dir` and `del` for what the host shell teaches. The primary commands are untouched; the owner has not decided whether to rename them. The existing `Disk_CatIsNotACommand` case is inverted rather than deleted, so the reversal is visible to whoever reads the test next
...
- [x] T060 `disk --help` is help. It reached the command table, was told `--help` is not a command, and answered a question about the grammar by refusing to run and complaining about the grammar, exit 2 for something the tool knows the answer to, because help was recognized only as `argv[1]`. A help request is now found anywhere in the disk arguments, in every spelling and either prefix, and resolves to `Command::Help`, which the runner answers with the disk section of the help on the output stream and a clean status. **`/HELP/STARTUP` stays a ProDOS path**: the spellings are matched exactly and in lower case. `Command::Help` is deliberately NOT a row in the command table; that table is swept to check every command is described in the help, and a help request is not one of the things being described
- [x] T061 The unknown-command refusal names what is actually accepted. It listed the five original commands long after eight aliases were added, so a user who mistyped `catalog` was offered five words that did not include it. Built from `CommandLineParser::GetAllDiskCommands` now, and swept against that table by a test, so a command added to the grammar cannot go unoffered
...
- [x] T063 Command lines lead with every spelling, then one short description
      (`cat | catalog | dir | list | ls`), in place of a primary command with its
...
- [x] T064 The "put and get are named from the DISK's point of view" paragraph
      is deleted. It existed to rescue two command names the descriptions did not
      ...
      both directions are stated on their own command lines
...
- [x] T079 **Exit statuses are stated per mode, under each mode.** The single
      ...
      a DOS 3.3 `boot` naming a binary, bad command, missing file, unreadable
...
- [x] T102 **A test per route, in both prefixes, plus the size promise.**
      ...
      would print a general page over the command the runner answers, and asking
...
- [x] T117 **`run` names a surplus argument as one.** It was already refused,
      ...
      had not typed
- [x] T118 **`disk` refuses an operand its COMMAND has no slot for**, and the count
      is the command's own: `list` names a disk, every other command names a disk and
      a file. Two slots were filled whatever the command, so `disk list img.dsk
      ...
      exit 0. A command the table does not recognize is left alone, so the runner's
      "unknown disk command" is not preempted by a complaint about operand three
...
- **A disk option is accepted under every command, including the commands it does not
  ...
  `--type` and `--addr` on `list` are accepted and ignored at exit 0. Per-command
