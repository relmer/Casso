...
- Q: How should the UI drain timer behave when the debug window is hidden or minimized (but not paused)? → A: Continue draining the ring into the deque on every tick regardless of visibility; skip only the ListView repaint work (`ListView_SetItemCountEx`, auto-tail heuristic, and `ListView_EnsureVisible`) while hidden/minimized. Only `Pause` halts the ring-to-deque drain. This prevents ring overflow during long minimized intervals; the deque rolling cap (FR-012) is the sole memory bound.
- Q: When the user toggles a filter checkbox / radio or types in the Track/Sector boxes, when is the projection rebuilt — immediately, debounced, or only on the next drain tick? → A: Hybrid eager model (see FR-014d). Checkboxes and the Drive radio rebuild immediately on `WM_COMMAND` (UI thread `ListView_SetItemCountEx` + `InvalidateRect`), and the rebuild runs even while `Pause` is in effect. Track and Sector text inputs are eager-debounced: `EN_CHANGE` arms a one-shot timer with delay `kFilterTextDebounceMs = 250` ms; `EN_KILLFOCUS` flushes the pending timer immediately so leaving focus applies the filter without waiting.
...
- **FR-011 (UI drain timer)**: The UI thread MUST drain the ring on an approximately 30 Hz timer (every ~33 ms). Drains MUST be batched — one timer tick MUST drain all currently-available events from the ring into the deque (subject to the rolling-cap trim per FR-012). The ring drain into the deque MUST run on every tick regardless of window visibility — only `Pause` (FR-015) halts the ring-to-deque drain. When the window is hidden or minimized, the tick MUST skip only the ListView repaint work (`ListView_SetItemCountEx`, the auto-tail "at tail" check, and any `ListView_EnsureVisible` call per FR-013); on the next tick after the window becomes visible again, a single `ListView_SetItemCountEx` MUST be issued to reflect the deque size accumulated while hidden, and the auto-tail heuristic MUST resume on subsequent ticks. Rationale: keeping the drain running while hidden prevents ring overflow during long minimized intervals — the deque's rolling cap (FR-012) becomes the sole bound on memory growth, and no `[N events lost]` markers are produced solely because the window was hidden.
...
- **FR-014d (Filter state-change application timing)**: Filter state changes MUST be applied eagerly to the projection (no implicit deferral until the next drain tick), with two timing classes:
  - **Eager / immediate** — the event-type checkboxes (including the Audio master and its four sub-checkboxes), the Drive radio group, and the Track-filter "raw qt" toggle. The `WM_COMMAND` handler MUST, on the UI thread, rebuild the projection, call `ListView_SetItemCountEx` (with `LVSICF_NOINVALIDATEALL` per FR-013 conventions), and `InvalidateRect` the ListView before returning. The new filter state MUST be visible on the next paint, even while `Pause` (FR-015) is in effect (Pause halts ring-to-deque drain, NOT projection rebuilds).
  - **Eager / debounced** — the Track and Sector text inputs. `EN_CHANGE` MUST arm (or re-arm) a single one-shot timer with delay `kFilterTextDebounceMs = 250` ms; when the timer fires, the handler MUST rebuild the projection, `ListView_SetItemCountEx`, and `InvalidateRect` exactly as in the immediate case. `EN_KILLFOCUS` MUST flush any pending debounce timer immediately (cancel the timer and run the rebuild now) so that leaving the input applies the filter without further wait. The 250 ms value MUST be a named constant in the dialog's constants block (not a magic number at the call site).
  Both classes MUST run on the UI thread and MUST NOT touch the ring buffer or the deque's contents (filter changes are pure read-side projection rebuilds).
