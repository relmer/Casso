...
- [x] T038 [PH5] Create `Dxui/Theme/IDxuiTheme.h` per `contracts/IDxuiTheme.h.md`. Pure-virtual accessors for background, foreground, accent, focus ring, disabled foreground, caption colors, body / caption font handles, plus any others the contract sketch lists. All accessors `const` and return-by-value or `const &`. No `IDxui` body, interface only. **Exit**: header compiles standalone (only `#include "Pch.h"`). **FR**: FR-032.
...
- [x] T042 [PH5] Add a single smoke test `UnitTest/Dxui/MockDxuiThemeTests.cpp` that constructs `MockDxuiTheme`, constructs a `DxuiButton` with no D3D device, and asserts the button's `AccessibleName` / bounds setter / `SetVisible` behavior. Paint is **not** exercised yet (no painter mock until Phase 6). **Depends on**: T040, T041. **Exit**: test passes via `scripts\RunTests.ps1`. **FR**: FR-032; **SC**: SC-007 (foundation).
...
- [x] T071 [P] [PH7] Wire `DxuiHostWindow` to the existing `DxuiDwm` helpers (from T015) for backdrop, immersive-dark-mode, rounded-corner, and titlebar-color configuration. Apply on `WM_CREATE`, re-apply on `WM_DWMCOLORIZATIONCOLORCHANGED` / `WM_SETTINGCHANGE` / `WM_THEMECHANGED`. **Depends on**: T064. **FR**: FR-050, FR-051.
...
- **T079 `DxuiDropdown`**: Settings page dropdowns (`be96905`).
- **T080 `DxuiPopupMenu`**: Disk II / Input debug-panel column menus (`d82358e`).
- **T081 `DxuiTooltip`**: Disk II / Input debug-panel hover tooltips (`d111901`); a `DxuiPopupHost::MeasureText` seam sizes the balloon. (The `EmulatorShell` joystick tooltip stays in-window: its only drive path, `UiShell::Render`, went dead with the T129 swap-chain flip and needs its tick/render relocated to the host pump first.)
- **T117–T122 `DxuiMenuBar`**: the application main-menu submenus (`9ec5f90`), the real SC-008 beneficiary (its click-to-open submenus escape the window). Uses a no-capture popup so the owner keeps hover-switch between titles and arrow-key navigation; the owner drives dismiss (outside-click / caption / resize / move). `DxuiPopupHost` gained a `grabsCapture` flag and `WM_MOUSEACTIVATE` → `MA_NOACTIVATE` for this model.
...
**Phase 11d sub-step 1 (`ce09bcb`)**: `DxuiHostWindow` exposes shared `GetDevice()` / `GetContext()` / `GetSwapChain()` accessors so a consumer's renderer can target the host's swap chain without standing up its own.

**Phase 11d sub-step 2 (Session A, `092bb67` + `ff4ea01`)**: API additions only, no consumer flip yet.

  * `092bb67` feat(dxui): extend `DxuiHostWindow::CreateParams` with four optional fields (`classNameOverride`, `useInitialWindowRectPx` + `initialWindowRectPx`, `appIconBig` / `appIconSmall`) so a consumer can land its existing HWND-creation bespoke logic on `DxuiHostWindow::Create` without losing behavior. All four default to nullptr / false; existing callers see no change.

  * `ff4ea01` feat(dxui): add `IDxuiHostClient` interface + `DxuiHostWindow::SetClient` installer. Lets a full-ownership consumer receive the Win32 messages the host does not own end-to-end (commands, keyboard, mouse, paint, timer, drawitem, init-menu-popup, close, destroy, NC-LButtonUp with original hit-test). Eight new unit tests (1796 → 1804 passing). Build green on all four configs; code analysis clean.

**Phase 11d sub-step 3 (Session A wrap, `ab5b70f`)**: Switched `IDxuiHostClient` to a typed `DxuiMessageResult` enum (`Handled` / `NotHandled`) + `LRESULT` for rich-return messages. Polarity now matches Win32 / WPF / MFC convention; the legacy `Window`-base `bool` ambiguity ("which value means consumed?") is eliminated at the call site.

**Phase 11d sub-step 4 (Session B, `3f8b3d9` + `76ae14c` + `9acc8e1`)**: EmulatorShell HWND ownership flipped from `Window` base into full-ownership `DxuiHostWindow`. **SC-019 satisfied**.

  * `3f8b3d9` feat(dxui): added `CreateParams::createSwapChain` opt-out (default `true`) so a consumer that drives its own swap chain via a child HWND can ask the host to skip its own. EmulatorShell uses it; the existing `CassoRenderSurface` child HWND keeps owning `D3DRenderer`'s swap chain unchanged (Session C territory).

  * `76ae14c` refactor(casso): `class EmulatorShell : public Window, public IDriveCommandSink` → `class EmulatorShell : public IDxuiHostClient, public IDriveCommandSink`. Rewrote `CreateEmulatorWindow` to populate `CreateParams` (with `classNameOverride = L"CassoWindow"`, pre-computed `useInitialWindowRectPx` + `initialWindowRectPx` for saved-placement / monitor-aware sizing, `appIconBig` / `appIconSmall` for the WM_SETICON handoff, `createSwapChain = false`) and call `m_host = std::make_unique<DxuiHostWindow>(); m_host->SetClient (this); m_host->Create (params);`. Translated all ~13 `Window`-virtual overrides to IDxuiHostClient overrides with polarity preserved at every return site (`return true` → `NotHandled`; `return false` → `Handled`; OnNcLButtonUp's inverted polarity preserved correctly). Dropped `TryDelegateMessage`, dropped `SetInitialDpi`, added local `DxuiDpiScaler m_scaler` that LayoutManager references and `OnDpiChanged` keeps in sync with the host. Build green on all four configs; 1808/1808 tests passing.

  * `9acc8e1` refactor(casso): deleted `Casso/Window.{h,cpp}` (1153 LOC removed) and dropped them from `Casso.vcxproj`. EmulatorShell was the only consumer; `ChromedPanelWindow` uses adopt-mode `DxuiHostWindow` and never inherited from `Window`. **No production class still inherits from `Window`**.
...
**Session pickup note (2026-06-11), decision: proceed with the swap-chain flip (T129 steps 4-6)**:
  ...
  * **Chrome regression fixes committed + pushed this session** (do not redo): `1c8845a` fix(dxui), menu-item width cache (resize spacing collapse) + `WM_MOUSELEAVE` spurious-leave guard; `8435975` fix(casso/ui), caption-button press state via real `GetKeyState(VK_LBUTTON)` + clear menu hover on caption entry. These were partial band-aids; see below.
...
- [x] T133 [PH15] Move the WCAG color math off the theme into `Dxui/Theme/DxuiColor.{h,cpp}`; update call sites. **FR**: FR-112.
...
- [x] T162 [PH18] **Migrate `SettingsPanel` onto `DxuiPropertySheet`.** The hand-rolled `[OK][Cancel]` panel is a bespoke instance of the FR-129 framework; migrating it would fold FR-131 / FR-132 (notice + per-page dirty/Apply) into the generic sheet and delete bespoke Apply/Cancel/baseline bookkeeping. Sequence after T160 / T161, or fold them into the migration. **FR**: FR-129, FR-097. **Exit**: `SettingsPanel` derives from (or is replaced by) `DxuiPropertySheet`; bespoke dirty/Apply/Cancel plumbing removed.
  - **Scope note (investigated 2026-07):** larger than "swap a base class." `SettingsPanel` is a `DxuiPanel` hosted by the bespoke `SettingsWindow` (raw Win32 `WndProc` + `DxuiHwndSource` adopt mode + `SettingsWindowRenderer` blur/live-preview compose, FR-041); `DxuiPropertySheet` is a `DxuiWindow` that owns its own tab strip + **OK/Cancel/Apply** row and applies-all-dirty-on-OK. Full migration = rehost off `SettingsWindow` onto `DxuiWindow` (no framework analog for the blur/live-preview compose) **and** reconcile the sheet's Apply-button / apply-on-OK model against the shipped no-Apply / "OK (reboot)" (FR-131) / Theme "Apply now" (FR-132) UX, plus move the cross-cutting `SettingsApplyController` commit into per-page `OnApply`, plus unify the `UiShell` WM_* input shims onto the uniform Dxui dispatch. Chosen approach: **incremental slices** (user decision).
  - **Slice 1, DONE:** the four setting pages (`MachinePage` / `HardwarePage` / `ThemePage` / `DisplayPage`) now derive from `DxuiPropertyPage` (was `DxuiPanel`), each passing its tab title (`Machine` / `Hardware` / `Theme` / `Display`) to the base ctor. This gives every page the `Title()` + dirty flag + `OnActivated()` / `OnApply()` hooks (inherited, dormant for now) that a later slice drops into a `DxuiPropertySheet`. Behavior-preserving: `SettingsPanel` + `SettingsWindow` + `SettingsApplyController` unchanged; the bespoke tab strip / OK-Cancel / apply pipeline still drive the pages. Verified: Casso + UnitTest build clean (Debug x64); DPI-aware screenshot confirmed the Settings dialog opens (Ctrl+,) with all four tabs rendering and the Machine page fully populated (no UX change).
  - **Slice 2, DONE (framework-prep):** extended `DxuiPropertySheet` so it can express Casso's shipped button model before the rehost consumes it. Added `SetApplyVisible(bool)` (drop the Apply button → a plain right-aligned `[OK][Cancel]` sheet that commits only on OK, honored at `OnCreate`), `SetOkText(std::wstring)` + `SetOkWidthDip(int)` (relabel/resize OK for FR-131's "OK (reboot)", size for the longest label up front, then relabel with just a repaint), and a pure static `LayoutButtonRow(...)` helper that encapsulates the right-aligned reflow (hidden Apply, custom OK width), the `ContainerSizeForFill` idiom, unit-testable without a window. FR-132 "Apply now" needs no sheet API (pages are `DxuiPanel`s and host their own button). **Framework-only, no Casso consumer yet**, so the six `DxuiPropertySheetTests` (defaults, setters, three-button/hidden-Apply/custom-width/192-dpi reflow) ARE the verification. Suite 1953/1953 green (Debug x64). Rationale: the originally-planned "wire per-page dirty/OnApply" was found to be dead code until the sheet hosts the pages, `SettingsApplyController::CommitApply` is load-bearingly cross-cutting (ordered theme→machine switch; hardware-reset drives the switch) and per-page dirty has no consumer under the no-Apply model, so that work was folded into the rehost and replaced with this framework-prep slice (user decision).
  - **Slices 3a–3d + compositor, DONE:** `SettingsSheet : DxuiPropertySheet` replaces `SettingsPanel`; `SettingsWindow`, `SettingsWindowRenderer`, and `SettingsPanel` are **deleted**, and `IDM_VIEW_SETTINGS` (`Ctrl+,`) opens the modeless, **composited** sheet. The no-Apply / "OK (reboot)" (FR-131) + Theme "Apply now" (FR-132) UX is preserved via `SetApplyVisible(false)` + OK relabel. The blur / live-preview compose was ported to a new `Casso/Ui/Settings/SettingsCompositor.{h,cpp}` and installed as the composited window's **compose hook** (base renders panel + caption + color-picker overlay to an offscreen texture → separable Gaussian blur → per-pixel-alpha emulator reveal over the overlap + sharp feathered focus control). `SettingsSheet::UpdatePreviewCompose` drives the per-frame transparency state (active flag + `EmulatorShell::EmulatorContentScreenRect` overlap rect + `DisplayPage::FocusedControlRect`). Verified end-to-end by DPI-aware screenshot automation: the inactive path renders identically to a plain opaque window; a Display-control drag blurs+dims the panel, keeps the dragged control sharp, and reveals the running emulator through the overlap; snaps back sharp on release. Build clean (0 warnings); 1959 unit tests green. Commits `5681b0d` (compositor core) → `a84fece` (compositor wiring).
  - **Exit deviation (deliberate):** the cross-cutting `SettingsApplyController` commit was **not** dissolved into per-page `OnApply`; its ordering is load-bearing (theme → machine switch; hardware-reset drives the switch), so `SettingsSheet::OnOk`/`OnCancel` delegate to it instead, and `SettingsApplyController` + `SettingsPanelState` are retained as helpers rather than deleted. Input is unified on `DxuiWindow`'s own dispatch (the sheet owns its HWND); the legacy `UiShell` WM_* Settings shims are gone with `SettingsWindow`/`SettingsPanel`.
...
- **After Phase 7**: `DxuiHostWindow` + chrome primitives + DWM wiring exist as framework code; no behavior change yet.
...
- **After Phase 10**: `DxuiMenuBar` widget and `MainMenu` rename land, internal cleanup, no behavior change.
...
- **CHANGELOG.md / README.md updates** are user-facing: T110 in Phase 11 is the explicit reminder, but every phase that flips a user-visible behavior (Phase 7 snap-layouts, Phase 8 dropdown fix) should also touch `CHANGELOG.md` at minimum.
