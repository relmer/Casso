...
- [ ] T089 [PH12] **TRUE REMAINDER (not blocked)** — Route the emulator viewport's key/mouse through `DxuiViewport::SetInputSink` / `IDxuiViewportInputSink` with `consumesInput = true`. Prior `LayoutManager`/`IEdgeContributor` blocker rationale is obsolete and removed; the deferral is genuine: as of the T129 landing the emulator viewport is wired for rendering (bounds → `D3DRenderer::SetTargetBounds`) but input is still routed through the legacy `EmulatorShell` path, not `DxuiViewport::SetInputSink` (verified: `rg -n 'SetInputSink|IDxuiViewportInputSink|consumesInput' Casso/` → 0 hits). **Depends on**: T084. **FR**: FR-034. **Exit**: `rg -n 'SetInputSink' Casso/` returns ≥1 hit and the emulator viewport reports `consumesInput == true`. _The sink interface and viewport forwarding are ready and unit-tested (T083/T084)._
...
- [ ] T160 [PH18] **Implement FR-131 — staged machine/ROM restart notice.** On the Settings Machine page (machine change) and ROM/Hardware page (ROM change), show an inline notice that names the changed setting(s), states they take effect after **OK**, and warns that OK restarts the machine. The notice clears when the staged change is reverted. Commit model unchanged (`[OK][Cancel]`, no Apply button). **Files**: `Casso/Ui/Settings/MachinePage.{h,cpp}`, `HardwarePage.{h,cpp}`, `SettingsPanel.{h,cpp}`, `SettingsApplyController.{h,cpp}` (query staged machine/ROM diff). **FR**: FR-131. **Exit**: notice appears / clears correctly against staged diffs (manual); a unit test asserts the notice model reflects staged machine/ROM changes.
- [ ] T161 [PH18] **Implement FR-132 — Theme "Apply now" button.** Add an "Apply now" button immediately to the right of the theme dropdown on the Theme page; a theme applies on **OK** OR on **"Apply now"** (live, without closing); Cancel after "Apply now" reverts the theme to the panel-Show baseline. **Files**: `Casso/Ui/Settings/ThemePage.{h,cpp}`, `SettingsPanel.{h,cpp}`, `SettingsApplyController.{h,cpp}` (reuse `StagePendingTheme`; add a live-apply that snapshots the baseline for revert). **FR**: FR-132. **Exit**: "Apply now" live-applies; Cancel reverts to baseline (manual); a unit test covers the apply / revert baseline.
