...
**Status (this session — partial)**: Chrome reshape landed (T123–T128). EmulatorShell restructure (T129) is partially landed (Sessions A through C); the phase exit gate (T130) is deferred to Phase 11e. NC duplicate count is still **2** after Session C (the chrome surface drop from T127 stands; the EmulatorShell-side drop to 1 is gated on the Session C Step 4/5 work below). The full Phase 11 NC target (2) is therefore met for the chrome surface; the further drop to 1 lands once the host's panel-tree paint pump exists and chrome controls move into the root panel.
...
**Still deferred from Session C (Steps 4 + 5 of the Session C plan)**: the actual swap-chain flip and `CassoRenderSurface` deletion. Two pre-requisites are missing that put visual regression risk above the "no visual verification" threshold the agent could clear:

  1. `DxuiHostWindow::WndProc`'s `WM_PAINT` handler does not yet walk the panel tree — it only forwards to `IDxuiHostClient::OnPaint`. The host's RTV is also never created (the `GetBackBufferRtv()` accessor still returns `nullptr` even in full-ownership mode, per the comment on the accessor declaration). Activating the chrome paint pump therefore requires (a) RTV creation in `CreateRenderResources`, (b) a paint-tree walk on `WM_PAINT` (`BeginPaint` + bind RTV + `m_painter.Begin` / `m_root->Paint(...)` / `m_painter.End` + invoke `m_beforePresentHook` + `m_swapChain->Present` + `EndPaint`), and (c) DPI/theme threading down through the painter.

  2. Chrome controls (TitleBar, MainMenu, the two DriveWidgets, JoystickToggleButton) are not yet added to `m_host->Root()` as children — they're painted directly by EmulatorShell via `D3DRenderer`'s after-blit hook (`m_uiShell.Render()` in `EmulatorShell.cpp` ~line 762). Flipping the swap chain without first moving them into the panel tree would erase all chrome at frame 0.

  Both items together are the *real* Session C: they reshape DxuiHostWindow into a self-paints-its-children host AND restructure how EmulatorShell hands its chrome controls over. That's a substantially larger refactor than the originally-scoped "swap and delete" flip. **No visual verification was attempted** because the underlying pump is missing.

**Still required for Session D (SC-018 line-count target)**: helper-class extraction inside `EmulatorShell.cpp` to push line count `≤ 2,502`. Current line count is unchanged from baseline (Session C added ~150 lines for viewport plumbing + helpers but extracted nothing).
...
- [~] T129 [PH11d] **EmulatorShell restructure (the big one)**: change `EmulatorShell` to no longer inherit from `Window`. Either (a) `EmulatorShell` composes a `DxuiHostWindow` and forwards its main loop to it, or (b) split `EmulatorShell` into `EmulatorShell` (content controller) + a separate `Window`-style adapter — choose whichever is cleaner. Move HWND ownership from `EmulatorShell` (or `Casso/Window.cpp` wherever it lives) into `DxuiHostWindow` (now in **full-ownership mode**, not adopt mode). Move swap-chain ownership into `DxuiHostWindow`. Move the D3D11 device wherever makes sense — likely `DxuiHostWindow` owns the device that paints chrome, or the device is shared between EmulatorShell's viewport renderer and `DxuiHostWindow`'s chrome renderer. Reroute Apple ][ viewport rendering through `DxuiHostWindow`'s swap chain via the minimal `DxuiViewport` placeholder from T128. **Depends on**: T070, T112, T123–T128. **FR**: FR-105, FR-107, FR-108. *(Partially landed across Phase 11d Sessions A → C. **Session B**: HWND ownership moved into `DxuiHostWindow` in full-ownership mode, `EmulatorShell` no longer inherits from `Window` (SC-019), `Window.{h,cpp}` deleted. **Session C**: `DxuiHostWindow::SetBeforePresentHook` API, DxuiViewport child wired into host root panel with bounds-changed → `D3DRenderer::SetTargetBounds`, `D3DRenderer::Initialize2` + `UploadAndComposite` for host-owned swap chain. **Still deferred**: the actual swap-chain flip itself (Steps 4 + 5 of the Session C plan) — the host's `WM_PAINT` does not yet walk the panel tree, chrome controls are not yet children of `m_host->Root()`, and visual verification was not available. See the Session C deferment notes above for the full pre-requisite list.)*
