...
- **FR-011b**: On the //c (no parallel card), the printer path MUST route serial **port 1**'s 6551 transmit stream into spec 015's platform-neutral printer pipeline (via `PrinterByteRing`), producing rendered output equivalent to the //e parallel path for the same document. This requires the printer ring to be reachable independently of any `PrinterCard` (a machine-level sink) plus an `AciaPrinterEndpoint` bridge; the interpreter, raster, and PNG stages are reused from 015 unchanged. See `serial-printer-integration.md`.
- **FR-011c**: The **Hardware settings tab** MUST let the user choose which emulated device/endpoint is attached to each configurable port. For the //c: serial **port 1** (ImageWriter II printer / terminal loopback / raw-file capture / disconnected) and serial **port 2** (modem loopback / raw-file / disconnected). The selection MUST persist per machine (user config) and be applied at machine build, overriding the profile's default endpoint. This extends the Hardware tab's current enable/disable device tree (`HardwareEntry` in `SettingsPanelState`) with a per-port attachment selector; it also lays the groundwork for slot-device attachment on slotted machines (out of scope for v1 beyond the //c serial ports).
