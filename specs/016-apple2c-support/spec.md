...
- **FR-011b / FR-011c → deferred to issue #87** (`Apple //c serial printer integration + per-port device selection`): the //c serial **printer** front door (route port 1's 6551 TX into spec 015's platform-neutral pipeline via a machine-level `PrinterSink` + `AciaPrinterEndpoint`) and the Hardware-tab **serial endpoint selector** (per-port dropdown: printer / terminal loopback / raw-file / disconnected, persisted per machine). Both need spec 015 on `master` **and** a bootable //c, so they follow this spec. This spec keeps the ACIA + both serial ports working via loopback/file endpoints. See `serial-printer-integration.md`.
