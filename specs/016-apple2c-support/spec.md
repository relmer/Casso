...
- **FR-011b / FR-011c → out of 016 scope; owned by downstream issue #87** (`Apple //c serial printer integration + per-port device selection`): the //c serial **printer** front door (route port 1's 6551 TX into spec 015's platform-neutral pipeline via a machine-level `PrinterSink` + `AciaPrinterEndpoint`) and the Hardware-tab **serial endpoint selector** (per-port dropdown: printer / terminal loopback / raw-file / disconnected, persisted per machine). These are **not** 016 requirements — #87 depends on 016 **and** spec 015 landing on `master`, so it is strictly downstream of this spec (016 never waits on #87). This spec's serial scope is the ACIA + both ports working via loopback/file endpoints. See `serial-printer-integration.md`.
...
- **FR-014a**: The //c **external drive** MUST be user-connectable via a **Connected / Not connected** toggle on the Hardware settings tab; connecting reveals the existing drive-mount widget (mount / eject / write-protect) and the state persists per machine. No new media machinery — it reuses the existing disk-mount flow.
