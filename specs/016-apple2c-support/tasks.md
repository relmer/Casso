...
- [~] T030a [US4] Add `Mouse` to `InputMappingMode` + segmented skeuomorphic device selector. *(**Model slice done**: `InputMappingMode::Mouse` added (`GlobalUserPrefs` "mouse" token round-trips; persisted), the cycle toggle gains Mouse **gated to mouse-capable machines** (Off→Joystick→**Mouse→Paddle** — Mouse precedes Paddle because Paddle's capture makes the toggle unclickable), `JoystickToggleButton` shows a "Mouse Mode" label + tooltip, and the cursor-hide/button-capture are gated on **GuestMouseLive** (firmware actually ENBXY'd the IOU — hardware truth garbage RAM can't fake), so Mouse mode is invisible until mouse software runs.)*
