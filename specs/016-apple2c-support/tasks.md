...
- [ ] T024 [US3] Wire the two `Acia6551` instances (Phase 2) into the //c serial ports (slots 1 + 2) in `Apple2c.json` + the //c serial firmware, with **loopback/file** endpoints. *(US3's //c-specific integration — the **complete 016 serial deliverable**: self-contained, testable via loopback, no dependency on 015 or #87. The printer-endpoint bridge is **not** part of T024.)*
