...
- [~] T030a [US4] Add `Mouse` to `InputMappingMode` + segmented skeuomorphic device selector. *(**Model slice done**: `InputMappingMode::Mouse` added (`GlobalUserPrefs` "mouse" token round-trips; persisted), the cycle toggle gains Mouse **gated to mouse-capable machines** (Off→Joystick→Paddle→Mouse only when the //c mouse exists, else wraps to Off), and `JoystickToggleButton` shows a "Mouse Mode" label + tooltip. **Remaining**: replace the cycle-toggle with the segmented **skeuomorphic-icon** selector (joystick / paddle / mouse) — needs icon assets.)*
