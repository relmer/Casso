...
- [ ] T066 [US1] Create `CassoEmuCore/CassoExplorer/CassoExplorerCommands.h/.cpp`: the `DxuiCommand` table for the command bar, context menus and keyboard; `isEnabled` functors read `BrowserModel` selection and image write-protect state. The menu bar it first fed is retired by T109 -- status 2026-09-16: table, keys and toolbar rows done; the write-protect enable check is unconfirmed
