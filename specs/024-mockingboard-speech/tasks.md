...
- [ ] T002a **Decide two premises the cross-check put in doubt** (research.md PENDING-2) — needs a human call, not another source: (1) how many voice chips and PSGs the emulated model actually has, since the lettered A/B/C/D names and the Sound-I/Sound-II/Sound-Speech-I names appear to be different schemes the community conflates; (2) **SC-01 or SSI-263**, now that a period source describes Sweet Micro's own speech boards as carrying the Votrax SC-01. Neither blocks the decode; both affect the data model and the UI naming
