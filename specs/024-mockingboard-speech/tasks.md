...
- [ ] T001a Close T001's two residual gaps from the datasheet **page images** (the OCR text cannot supply them): the exact bit positions of the inflection fields split across registers 1 and 2, whose *Register Input Formats* table is a graphic; and the phoneme symbols for `$3A`–`$3D` plus a check of the full chart against the page
