...
- [~] T032 [US1] PARTLY DONE -- the emulator was launched and booted a real nibble image, and the filter agreement is covered by tests; the create dialog's dropdown was NOT driven by hand. Confirm by hand that the disk picker and drag-and-drop offer `.nib` and `.nb2` with no change to `Casso/Ui/DriveWidgetState.h`, launching `x64\Debug\Casso.exe --machine Apple2e`. Not a unit test: a test may not read the source tree, and T023 covers the part that is mechanically checkable
