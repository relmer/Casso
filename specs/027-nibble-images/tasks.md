...
- [ ] T005 Declare `NibbleImageCodec` in `CassoEmuCore/Devices/Disk/NibbleImageCodec.h`: the two accepted total lengths and their track sizes as named constants, the sync byte, `Load`, `Serialize`, and a length-to-geometry resolver. Terse one-line comments only; the documentation blocks belong in the `.cpp`
