...
- [X] T030 Retire the 2D skeuo paths (gated on T029) — SCOPE NARROWED by user decision: the scene keeps an opt in/out while the 3D models mature and drive docking is still to come. Done: deleted `Casso/Ui/Chrome/MonitorFrame.h/.cpp` + all references. Kept deliberately: the Settings > Theme toggle, now bound to the new `deskScene` pref (default ON; replaces the retired `skeuoMonitorFrame` JSON key, which is ignored on load) as the escape hatch back to the classic bare display + 2D drive band; `DriveWidget`'s non-compact paint path (the opt-out band needs it); the effective-DPI scene-scale fold + settle loop (inert at scale 1.0, live for the scene branch)
