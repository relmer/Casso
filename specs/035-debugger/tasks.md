...
- [ ] T227 Give every Casso command a help category (running and stepping, breakpoints, registers and flags, memory, disassembly and data, symbols and source, disks, display and panels, session and settings), its syntax as typed and a one-line description, in one table beside `CassoCore/Debugger/AppleWinCommandTable.cpp`, and give the dialect tables in `CassoCore/Debugger/CommandModeHelp.cpp` categories too, per FR-123 (missing); AppleWin's commands get the descriptions they lack
- [ ] T228 Record, per dialect (AppleWin, Monitor, GSSquared, WinDbg), which Casso commands it has an equivalent for and which Casso names it already uses for something else, per FR-014, FR-122 and the "Casso commands and equivalents" assumption (missing); tested so the Casso section of each dialect's help is exactly the Casso commands it has no equivalent for and does not use
- [ ] T229 Route every Casso command a dialect has no equivalent for through that dialect's marker, not only the Engine family, per FR-014 and US12/AC8 (partial): `GSSquaredParser`, `WinDbgParser` and `MonitorParser` ask T228's table rather than `IsEngineCommand`; `DISK` runs in GSSquared; a name the dialect already uses keeps its dialect meaning
- [ ] T230 Add the Casso command mode per FR-011 and US12/AC9 (missing): `CommandMode::Casso`, `MODE CASSO`, an entry in the command bar's Dialect menu, AppleWin's output format, and every command in the table runnable by bare name
- [ ] T231 Rebuild HELP in `ConfigHandlers::Help` per FR-122 and FR-123 (partial): the dialect's own commands by category, alphabetical within each, syntax then description; then "Casso commands" in the same layout, each written as it is typed in that dialect; Casso mode lists every command with no separate section; AppleWin mode separates AppleWin's commands from Casso's additions
- [ ] T232 Answer `HELP <command>` for a command the dialect cannot run by naming the dialect that has it, per FR-124 (contradicts): today GSSquared's `help disk` describes DISK as if it ran
- [ ] T233 Test per SC-031 that in every mode each command help lists runs when typed as listed, and that every command the mode can run is listed (missing)
