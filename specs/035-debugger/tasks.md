...
- [X] T220 Add a per-row checkbox to `Dxui/Widgets/DxuiListView.{h,cpp}` per FR-117 and US17/AC9 (missing): a `Cell` flag that draws a themed checkbox ahead of the cell's icon and text, a hit test, and a callback when it is clicked or toggled with Space, with tests beside the list's others; the breakpoints pane's first column shows it, checked while enabled, followed by the FR-092 mark, and toggling it sends `BPE`/`BPD <id>` **Done 2026-09-25:** `DxuiListView::Cell::check` draws a checkbox ahead of the icon; a press on it, or Space on the selected rows, reports the toggle through `SetOnCheckToggled` without moving the selection, with `DxuiListViewCheckTests`. The breakpoints pane lost its gutter column; each row's cell carries the checkbox, the mark and the text. Checked in the window.
...
**Purpose**: The 2026-09-25 help and dialect amendment (FR-011, FR-014, FR-122 to FR-124, SC-031, US12/AC8-9). Help lists only what can be typed, in one layout, and a Casso dialect holds the full native set.

- [X] T227 Give every Casso command a help category (running and stepping, breakpoints, registers and flags, memory, disassembly and data, symbols and source, disks, display and panels, session and settings), its syntax as typed and a one-line description, in one table beside `CassoCore/Debugger/AppleWinCommandTable.cpp`, and give the dialect tables in `CassoCore/Debugger/CommandModeHelp.cpp` categories too, per FR-123 (missing); AppleWin's commands get the descriptions they lack **Done 2026-09-25:** `CassoCore/Debugger/CassoCommandReference.{h,cpp}`: 173 commands, each with its category, its syntax as the parser accepts it and a description; names that only report (not available, or of no effect in a window showing every pane) have none. `CassoCommandReferenceTests`.
- [X] T228 Record, per dialect (AppleWin, Monitor, GSSquared, WinDbg), which Casso commands it has an equivalent for and which Casso names it already uses for something else, per FR-014, FR-122 and the "Casso commands and equivalents" assumption (missing); tested so the Casso section of each dialect's help is exactly the Casso commands it has no equivalent for and does not use **Done 2026-09-25:** each dialect help entry carries the Casso commands it stands for; `CommandModeHelp::IsWordOfMode` knows the words a dialect uses, GSSquared's hex words among them.
- [X] T229 Route every Casso command a dialect has no equivalent for through that dialect's marker, not only the Engine family, per FR-014 and US12/AC8 (partial): `GSSquaredParser`, `WinDbgParser` and `MonitorParser` ask T228's table rather than `IsEngineCommand`; `DISK` runs in GSSquared; a name the dialect already uses keeps its dialect meaning **Done 2026-09-25:** GSSquared and WinDbg parsers ask `CommandModeHelp::IsCassoCommandReachable`; every Casso command whose name the dialect does not use runs through its marker, its own forms included so the window's controls and scripts work in every dialect; `disk` runs in GSSquared.
- [X] T230 Add the Casso command mode per FR-011 and US12/AC9 (missing): `CommandMode::Casso`, `MODE CASSO`, an entry in the command bar's Dialect menu, AppleWin's output format, and every command in the table runnable by bare name **Done 2026-09-25:** `CommandMode::Casso`, `MODE CASSO`, a Dialect menu entry, the `Casso>` prompt, AppleWin's output format.
