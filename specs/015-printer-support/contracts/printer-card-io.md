...
| +$1 | read | Status: ready bit(s) asserted (never busy) |
| +$1..+$F | write | Ignored (no side effects) |
| +$2..+$F | read | Status alias (tolerant decode, R-001) |

Status byte: all commonly tested ready conventions asserted simultaneously
(exact value locked after Print Shop capture verification, R-001); the value
is constant — no state machine is observable from the guest.
