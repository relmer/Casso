...
- [X] T049 [US6] End-to-end: `PR#1` + `LIST` and DOS 3.3 `CATALOG` (quickstart scenario 6; SC-004) — USER-VALIDATED 2026-07-17: `PR#1` + `CATALOG` prints readable draft text with logic-seeking head motion and per-line clacks; `LIST` skipped as the identical path (PR#1 → firmware → draft font). Form feed (head-parked slew) and the responsive/themed toolbar passed the same session.
