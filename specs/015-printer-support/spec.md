...
- Q: Is the panel's skeuomorphic printer rendered as flat 2D shapes (like today's drive widgets) or true 3D? → A: True 3D perspective, built as a contained *additive* path on the existing Dxui D3D11 pipeline (an MVP-matrix constant buffer, one textured/lit shader, and chassis + curled-paper meshes) — not a new engine and not an image asset. It is the pilot 3D primitive the drive widgets adopt when they later move to 3D. The printed content (strip + paper furniture + ink) renders to a 2D texture that the 3D layer maps onto the paper surface, so the content pipeline stays 2D and unit-testable and a flat 2D fallback is always possible.
...
- **FR-034**: The paper-feed reveal MUST apply ink by print-head column position — the head sweeps left→right laying down ink per column, in step with FR-031 pacing — rather than revealing whole lines at once, so the user sees the head printing across the page. This requires the presentation layer to observe head column and timing, which the interpreter/worker MUST expose without altering the (immediately-complete) raster.
