...
- Q: Is pending output one continuous strip or a queue of discrete jobs? → A: Single open strip — one paper per machine; all output appends to it; eject delivers the entire strip; a canceled delivery leaves it pending. No job queue.
...
- Q: Is the delivery destination still a single setting chosen in Settings, delivered at eject? → A: No. Delivery is now per-action from the printer panel and non-destructive: **Print** sends the current strip to a Windows printer, **Save** writes a PNG through a file dialog, and **Copy** places it on the clipboard — each acts on the strip and leaves the paper loaded, so one printout can be printed AND saved AND copied without re-printing. There is no stored destination preference and no separate "eject": the strip is cleared only by the tear-off **Discard** action. Settings → Printing therefore keeps only output resolution and dot-rendering style; the "output destination" dropdown and PNG-folder picker are removed (Save defaults its dialog to `<Pictures>\Casso Prints`). This supersedes the 2026-07-07 answers that framed the destination as a global persisted setting delivered at eject.
- Q: What does the panel's Form Feed button do now that there is no eject? → A: It advances the paper to the top of the next page (a real ImageWriter FORM FEED), honored only while the print is idle so it never interleaves a page break into the guest's own stream. Delivery is the Print / Save / Copy buttons; tear-off is Discard.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Print a Print Shop Page to a PNG File (Priority: P1)

An emulator user boots an unmodified The Print Shop disk on the emulated Apple //e, walks through Print Shop's printer setup (ImageWriter II on an interface card in slot 1), designs a greeting card or sign, and chooses Print. The emulated printer receives the guest's print data and renders it as it would have appeared on paper; the finished page appears in the printer preview, where the user saves it as a PNG image file.
...
1. **Given** a machine configured with the printer interface card in slot 1, **When** Print Shop prints a single-page sign and the user chooses Save in the printer preview, **Then** a PNG file of the complete page is written to the chosen path and its content is a faithful monochrome rendering of the designed sign.
...
### User Story 3 - Delivering the Printout: Print, Save, and Copy (Priority: P3)

The printer preview offers three on-demand delivery actions, each acting on the current strip and leaving the paper loaded (non-destructive), so one printout can be delivered several ways without re-printing: **Print** sends it to a Windows printer via the standard Windows print dialog (selecting "Microsoft Print to PDF" there yields PDF output with no dedicated PDF support in the emulator); **Save** writes a PNG through a file dialog (defaulting to `<Pictures>\Casso Prints`); **Copy** places it on the Windows clipboard. There is no pre-selected destination and no separate eject — delivery is whichever button the user presses.

**Why this priority**: Multiple delivery targets multiply the value of the pipeline, but any single one (Save to PNG, from User Story 1) already proves it. Windows printing and clipboard copy are additive conveniences.

**Independent Test**: With a rendered strip on the paper, exercise Print, Save, and Copy in turn and verify each delivers the same strip while the paper stays loaded.

**Acceptance Scenarios**:

1. **Given** rendered output is on the paper, **When** the user presses the panel's Copy control, **Then** a bitmap of that output is on the Windows clipboard and pastes correctly into a standard image editor; the paper stays loaded so the same printout can still be printed or saved.
2. **Given** rendered output is on the paper, **When** the user presses Print, **Then** the standard Windows print dialog appears once, and confirming it prints the strip's pages (choosing "Microsoft Print to PDF" produces a PDF); the paper stays loaded afterward.
3. **Given** the Print or Save dialog is open, **When** the user cancels it, **Then** no output is produced, the paper stays loaded unchanged, and emulation continues unaffected.
4. **Given** a rendered strip that has been printed or saved, **When** the user presses Save (or Print, or Copy) again, **Then** it is delivered again from the same loaded paper — delivery never consumes the strip.

---

### User Story 4 - The Printer on the Desk (Priority: P4)

When the printer card is enabled, a small printer indicator sits in the window chrome, clear of the centered disk drives. The full skeuomorphic printer — an ImageWriter-class body with its four-color ribbon cartridge visible and paper rising from the platen showing the actual rendered output — lives in a docked side panel shown on demand: it reveals itself automatically the moment the guest engages the printer (firmware activation such as `PR#1`, or the first data byte written to the card), and can be opened or closed any time via the indicator. The panel's Print / Save / Copy buttons deliver the current strip on demand and leave the paper loaded; Form Feed advances to the top of the next page; Discard tears off and clears the strip. Closing the panel with output pending leaves the indicator signaling until the strip is discarded. Hovering either surface summarizes the virtual configuration.
...
3. **Given** un-delivered output is pending and the panel is closed, **Then** the indicator continues to signal the pending state; **When** the user reopens the panel and presses Print or Save, **Then** the strip is delivered and the paper stays loaded; **When** the user presses Discard, **Then** the strip is torn off and the paper clears.
...
The user prints a Print Shop banner — a message spanning many fanfold pages sideways. Saved as a PNG — or copied via the panel's Copy control — the banner emerges as one continuous image with no page-break seams, something no physical printer could produce. Printed to a Windows printer, the banner is split at page boundaries and printed as a sequence of pages.

**Why this priority**: Banners are a beloved Print Shop feature and the continuous-image rendering is a unique payoff of emulation, but it depends on the full pipeline plus delivery handling already being in place.

**Independent Test**: Feed a byte stream spanning several page lengths without interruption; verify the saved PNG is a single image of the full length and the printed output tiles the same content across pages.

**Acceptance Scenarios**:

1. **Given** a rendered banner strip, **When** the user saves it as a PNG (or copies it to the clipboard), **Then** a single image containing the entire banner is produced, with no seams, gaps, or duplicated rows at page boundaries.
2. **Given** the same rendered banner strip, **When** the user prints it to a Windows printer, **Then** the output is split into pages that tile the banner in order, together containing every row exactly once.

---

### User Story 6 - Text Printing from BASIC and DOS (Priority: P6)

At the BASIC prompt, the user types `PR#1` and then `LIST` (or `CATALOG` under DOS 3.3). The program listing prints as readable text in the printer's built-in font at the current character pitch, and the user delivers the page from the preview via Print or Save.
...
4. **Given** any recognition outcome, **Then** machine configuration (card enablement, slot) is unchanged.

---

### Edge Cases

- **Job without a final form feed**: Print Shop and BASIC listings often stop mid-page. The user delivers the strip on demand via the preview's Print / Save / Copy actions at any time, and the emulator must indicate when printed-but-undelivered data is pending.
- **Guest reset or reboot mid-print**: like a physical printer, pending page content survives a guest reset — and an emulator restart (persisted per machine); it is cleared only by the user's Discard action.
...
- **Output failure** (save path read-only, disk full, clipboard unavailable, printer/spooler error): the user is notified, the rendered strip is not lost (the paper stays loaded), and the user can retry after fixing the problem or choosing a different delivery action.
...
- **Print or Save dialog canceled**: the strip is retained unchanged and the paper stays loaded (Acceptance Scenario 3.3).
- **Panel closed or unavailable while output is pending**: the indicator always carries the pending/activity state; Copy and Discard remain available through menu commands, and Print / Save are reached by reopening the preview panel. The indicator never disturbs the disk-drive widgets' centered composition, at any window size.
...
- **FR-002**: The card MUST accept guest-written data bytes and expose a readiness indication (the parallel interface's ready/busy line) such that a guest honoring the documented handshake never loses or reorders a byte. The readiness indication de-asserts while the emulated printer is mechanically printing, so a guest polling it is THROTTLED to real ImageWriter speed — exactly like the hardware interface — rather than dumping the whole job instantly and racing ahead of the presentation.
...
- **FR-011**: Settings MUST include a dedicated Printing tab hosting the host-print-service render options: output resolution (FR-028) and dot-rendering style (FR-027). These are global user preferences shared by all emulated machines — host print services are host resources, like the keyboard — and MUST persist across emulator sessions. The delivery target is NOT a stored preference: it is chosen per action by the preview's Print / Save / Copy controls (FR-013/FR-014, and FR-020), so the Printing tab has no destination selector or output-folder picker.
- **FR-012**: The Save action MUST write a PNG image of the current strip to a path the user picks through a file dialog, defaulting to `<Pictures>\Casso Prints` with a collision-free suggested name (e.g. timestamped). Save is non-destructive (the paper stays loaded); on write failure or cancel the user keeps the strip and can retry.
- **FR-013**: The printer panel MUST provide an opt-in Copy control that places the current rendered strip on the Windows clipboard. The copy MUST supply compressed image data (the registered "PNG" clipboard format) at the configured output resolution, plus a legacy bitmap (CF_DIB) fallback provided via delayed rendering and size-capped for very long strips — full fidelity always travels in the compressed format. Copy is non-destructive: it leaves the paper loaded so the same strip can also be printed or saved.
- **FR-014**: The Print action MUST present the standard Windows print dialog once per invocation; cancellation retains the strip (the paper stays loaded). Pages MUST print at true scale (one fanfold inch equals one printed inch), centered on the target page, clipping only if the target printable area is smaller than the 8-inch printable width. The user MUST be able to see what they are committing to before the strip is sent: the panel's paper view shows the rendered strip on fanfold paper with page-boundary perforations visible, and the Print flow states the resulting page count before the user confirms. PDF output is provided by the user selecting the system's "Microsoft Print to PDF" printer — the feature includes no dedicated PDF generation.
- **FR-015**: For Save (PNG) and Copy, a strip spanning multiple form lengths MUST render as one continuous image (fanfold paper), up to a maximum strip length of 60 fanfold pages; for Print (Windows printer), the same strip MUST be split at page boundaries with no lost or duplicated content.
- **FR-016**: There is exactly one open strip per machine: all guest output appends to it. The panel's Print / Save / Copy actions deliver that strip on demand and are non-destructive — each leaves the paper loaded, so one printout can be printed AND saved AND copied without re-printing. The strip is cleared only by the tear-off Discard action (FR-029); there is no separate "eject". A form feed marks a page boundary within the strip (used for Windows-printer pagination) and delivers nothing. The emulator MUST indicate when un-delivered output is pending.
...
- **FR-020**: The panel MUST reveal automatically on the guest's first engagement of the card (slot-firmware activation or first data byte written) and MUST be openable and dismissible by user action at any time. Indicator and panel MUST reflect printer state — idle, receiving data, pending un-delivered output, and output-delivery failure (error light) — and the pending state MUST remain visible on the indicator whenever the panel is closed. The panel MUST expose the delivery actions Print (FR-014), Save (FR-012), and Copy (FR-013) — all non-destructive — plus a Form Feed control that advances the paper to the top of the next page (honored only while the print is idle) and the tear-off Discard action (FR-029), which alone clears the strip.
...
- **FR-018**: Printing MUST NOT measurably degrade the EMULATOR's own performance — emulation speed, audio continuity, or video smoothness — while the guest is sending print data. (The GUEST is authentically throttled by the printer's ready line at real print speed per FR-002, exactly as on hardware — the printer holding the computer, not the emulator slowing down. Running the emulator faster prints proportionally faster.)
- **FR-026**: Pending un-delivered output MUST persist across emulator sessions, per machine: the rendered strip, paper-advance position, and page boundaries are restored when the machine is next opened, with the indicator/panel reflecting the pending state. Interpreter settings (pitch, spacing, color) still reset per FR-010. Loss of pending output on abnormal termination is acceptable.
...
- **FR-031**: The strip raster is PRODUCED at real ImageWriter II printing speed: the card applies backpressure via its ready line (FR-002) so the guest prints no faster than the mechanism, paced off the emulated clock (running the emulator faster prints proportionally faster). The panel's paper animation and printer audio follow the guest's real print-head position 1:1 — the head sweeps left→right laying ink by column (FR-034) within the live viewport (FR-033) — so the preview can never fall behind or jump-cut a backlog at any emulator speed. Eject, discard, and Copy operate on the raster produced so far at any time; eject/discard fast-forward any queued feed instantly.
...
- **Print Job**: The machine's single open strip (canonical rule: FR-016) — all rendered output since the last discard, with its page boundaries. Delivered on demand and non-destructively via the panel's Print / Save / Copy actions; survives guest resets and emulator restarts until the user discards it.
- **Printing Settings**: The persisted, global host-print-service render preferences on the Settings → Printing tab — output resolution and dot-rendering style. The delivery target is not stored here; it is chosen per action by the panel's Print / Save / Copy controls. Host print services are shared by all emulated machines; guest-side printer hardware is per-machine.
...
- **SC-003**: A multi-page Print Shop banner produces a single continuous image when saved as a PNG or copied to the clipboard — zero seams, zero missing or duplicated rows at former page boundaries — and the same content split cleanly into pages when printed to a Windows printer.
...
- **SC-007**: A single rendered printout can be delivered to multiple targets — printed to a Windows printer, saved as a PNG, and copied to the clipboard — without re-printing, because each delivery leaves the paper loaded.
