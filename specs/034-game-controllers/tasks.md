...
- [ ] T051 [US2] Fold the joystick and paddle segments out of `CassoEmuCore/Ui/Chrome/InputClusterEntry.h/.cpp`, leaving the mouse: the picker now carries that answer on its face, so the icons duplicate it. `GetSegmentCount`, `IsSegmentOn`, `m_modes`/`m_segments` indexing, `Paint`, `OnClick`, `GetTooltipAt` and `GetPickerItems` all assume 0 = joystick, 1 = paddle, 2 = mouse; the entry must also disappear entirely on a machine with no mouse rather than laying out zero segments
