...
| 11a | In multiplayer with two pads of one model, pick a different profile for each from the Profiles submenu; relaunch; open Settings and move Editing between the two pads. | Each player's section has its own header and check, each pad plays its own profile, both survive the relaunch, and the Profile drop-down follows the pad in Editing. | FR-028, FR-029 |
