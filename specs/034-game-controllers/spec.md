...
- Q: In what order are the picker's entries listed? -> A: Attached controllers first, then the keys and the mouse. A real controller plays these games better than either stand-in, and the stand-ins are always there to fall back to. It also puts the checked entry at the top once a controller is plugged in (FR-008b).
...
- Q: Does the picker's glyph need a distinct disconnected state, or do the label and tooltip carry it? -> A: The label and the tooltip carry it; the glyph gets no third state. The glyph follows the DEVICE, and a stand-in is a real device of some kind, so it is already telling the truth about what is driving; a disconnected variant would be describing a different controller than the one the label names, which is the one case where the two would disagree. The picker's face wears the stand-in's own name, the tooltip says which controller is chosen and that it is not connected, and the chosen controller's own row in the list keeps its "(not connected)" text (FR-008a, FR-008b, FR-013).
...
- Q: Which API reports a controller that is ready to read, rather than a raw HID interface arriving? -> A: Not Windows.Gaming.Input: Casso uses no WinRT, so its GamepadAdded and RawGameControllerAdded events are out. XInput and DirectInput have no arrival notifications at all, which is why detection is a HID notification followed by rescans (research R4). GameInput, which is native and COM-style rather than WinRT, reports devices connecting and disconnecting through IGameInput::RegisterDeviceCallback and remains a candidate. It would replace the whole backend rather than only its detection, and machines without the GameInput redistributable would need it. The choice waits on measuring whether the current path misses a Bluetooth reconnect for want of a notification or for timing.
...
- **FR-008b**: The picker MUST show the source that is driving rather than the word for what it is for: the device's description, or the keys or the mouse, and "Controller" when nothing drives the axes. A controller row for a controller that is not connected MUST be shown and MUST NOT be selectable. Source labels MUST be capped in length, and a device description's trailing vendor and product parenthetical dropped before capping, so the picker's width moves as little as possible on a strip where every entry's position depends on it.
...
- **FR-031**: The Machine menu entries and the toolbar's paddle-source and profile pickers and status indicator (FR-008, FR-008a, FR-013, FR-028) MUST be built on the command, dropdown and toolbar widgets from the Dxui command-widgets feature (032), which is on master.
