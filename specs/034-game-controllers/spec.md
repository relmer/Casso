...
- **Paddle games**: with the default absolute response, a self-centering stick returns the paddle to the middle when released. Rate response (FR-021a) makes the paddle hold its position, and the "Paddles" starting point sets up one player's paddle on one controller. Two players use a controller each, assigned to their own paddles (User Story 7); two people cannot share one controller.
