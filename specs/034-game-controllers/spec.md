...
- **FR-029**: The active profile MUST be remembered per controller unit, globally rather than per machine, so two controllers of one model can use different profiles and a profile can be used on any machine. If it no longer exists, Default MUST be used. A machine's active profile saved by an earlier build MUST pass once to that machine's saved controller, when that controller has no active profile of its own.
...
- **Active profile**: per controller unit and global, which of its model's profiles that controller uses.
