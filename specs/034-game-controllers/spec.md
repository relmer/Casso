...
- **FR-008**: The Machine menu and the toolbar input control MUST offer ONE picker for what drives the paddle axes, listing arrows-to-joystick, mouse-as-paddle and every attached controller as mutually exclusive entries, with the one in use checked. Picking any entry gives the axes to it and takes them from the others; the exclusivity is what the control shows rather than something that happens behind the user's back. The //c IOU mouse is NOT in this picker and keeps its own separate control: it drives a slot card, not the game port.
