...
- Q: Two players both on Xbox One S controllers: can they play different profiles? -> A: Yes. Profiles still belong to the model, but each controller unit carries its own active profile, so two pads of one model can play different profiles or the same one. In Settings, the Profile drop-down is the active profile of the controller in Editing: pick player one's controller and choose its profile, then player two's (FR-028, FR-029).
- Q: Is the active profile per machine or global? -> A: Global, keyed by controller unit. A user can still make //c and //e profiles, and can play either on any machine. The //c's only difference, no second joystick, is not something a profile can solve (FR-029).
...
**Independent Test**: multiplayer mode is turned on and two mock controllers fill the two player slots, one on each paddle. Moving each one moves only its own paddle, and each player's first button reaches their own button line, with both held at once.

**Acceptance Scenarios**:

1. **Given** multiplayer on a //e with two players on joystick 0 and joystick 1, **When** both sticks are moved at once, **Then** each player's paddles follow only their own controller.
2. **Given** those two players, **When** both hold their first button at once, **Then** PB0 reads player one and PB1 reads player two (FR-039).
3. **Given** those two players, **When** either presses a control their profile binds to PB1 or PB2, **Then** nothing reaches the game port: those bindings are kept in the profile and ignored while multiplayer is on (FR-039).
4. **Given** a //c, **When** the user fills a player slot, **Then** only joystick 0, paddle 0 and paddle 1 are offered, because the //c has no PDL2/PDL3 (FR-035).
5. **Given** player one on joystick 0, **When** the user offers player two a paddle player one already holds, **Then** it is not offered, and a saved setup that claims one is refused rather than played (FR-036).
6. **Given** two players, **When** one controller disconnects, **Then** only that player's paddles return to center and their button line releases; the other keeps playing without interruption.
7. **Given** multiplayer on, **When** the user picks a single source from the toolbar picker, **Then** multiplayer turns off for that machine, both player slots are kept, and the picked source drives the game port exactly as it did before the mode existed (FR-037).
8. **Given** a machine left in multiplayer, **When** it starts or is switched to with none of its players' controllers attached, **Then** the game port falls back to single-source play on whatever is attached, the saved setup is left alone, and plugging a player's controller back in returns the mode without the user setting it up again (FR-040).
...
- **Profile deleted while another controller has it active**: every controller of that model whose active profile it was moves to Default, and a controller whose remembered active profile no longer exists plays Default without recreating it.
...
- **FR-028**: Each controller in play MUST have its active profile selectable from a Profiles submenu on the command bar's paddle-source picker, without opening the controller settings, and the choice MUST take effect on the next sample without resetting the emulated machine. The submenu lists Default first, then the model's other profiles, then New... below a separator; New... opens the Controllers page with the New Profile dialog. With more than one controller in play each has its own section, under a header giving the player and the controller, with its own check. The submenu is absent while no controller is in play. In the settings, the Profile drop-down is the active profile of the controller in Editing.
- **FR-029**: The active profile MUST be remembered per controller unit, globally rather than per machine, so two controllers of one model can play different profiles and a profile plays on any machine. If it no longer exists, Default MUST be used. A machine's active profile saved by an earlier build MUST pass once to that machine's saved controller, when that controller has no active profile of its own.
...
- **FR-037**: Each machine MUST be in one of two modes, remembered per machine beside its controller selection. In **single-source** mode one selected controller, or the arrow keys, or the mouse drives the game port, exactly as before this mode existed. In **multiplayer** mode two player slots drive it: each slot names an attached controller and what it maps to -- joystick 0 (PDL0/PDL1), joystick 1 (PDL2/PDL3), or a single paddle PDL0-PDL3 -- and the selection drives nothing. A controller in neither slot plays nothing. Picking a single source from the paddle-source picker MUST turn multiplayer off for that machine, keeping both slots.
- **FR-038**: A player's controller MUST play its own profile, remapped onto what its slot maps to: the controller's own PDL0.. targets land on the slot's paddles in ascending order, so a slot mapped to a single paddle plays only the controller's PDL0 bindings and two players share one profile without a per-player copy. In single-source mode the one controller MUST claim PDL0 and PDL1 and no more, leaving the rest free; a controller with enough controls to drive four axes MUST NOT claim them by default.
- **FR-039**: In multiplayer mode each player MUST drive one button line: player one's PB0 bindings drive PB0 and player two's drive PB1. A player's PB1 and PB2 bindings MUST be kept in the profile and ignored while the mode is on, and PB2 is unused, so a two-player game that reads the two lines separately can tell the players apart. In single-source mode the one controller MUST drive PB0-PB2 as before, and button state MUST still combine with the keyboard and mouse sources (FR-014).
- **FR-040**: The saved multiplayer mode is the user's INTENT, and what the machine plays MUST be derived from it and from what is attached. While the mode is on and none of the controllers its filled slots name is attached, the machine MUST play single-source mode instead, selecting as FR-032 chooses, rather than leaving the game port dead. The saved setup MUST NOT be rewritten, so the mode returns on its own when a player's controller returns. One player's controller leaving MUST NOT end the mode: only that player's paddles center and their button line releases (SC-012), because single-source play would otherwise hand their paddles to the player still holding a controller. Anything that reports the mode to the user, the toolbar picker above all, MUST report the mode as played rather than as saved.
...
- **Controller selection**: per machine, which controller (if any) drives the game port in single-source mode. Refers to a controller identity that may or may not currently be attached.
- **Multiplayer mode and its two player slots**: per machine, whether two people play and, for each of the two players, the controller they hold and what it maps to (FR-037). The slots may not repeat a controller or overlap their paddles (FR-036).
...
- **Active profile**: per controller unit and global, which of its model's profiles that controller plays.
...
- **Persistence location**: the controller selection lives with the existing per-machine input preferences; profiles, active profiles, calibration and deadzone are global and keyed by controller model or unit, since a stick's physical quirks and the user's layouts for it do not change with the emulated machine.
