...
- **Hardware check**: power a wireless Xbox controller on and off (Xbox wireless adapter or Bluetooth, and an Xbox 360 wireless receiver if available) and confirm a HID arrival and removal notification is raised each time. **UNVERIFIED** for the 360 receiver, which may expose its four controller endpoints permanently. **Fallback** if a receiver raises nothing: while a receiver of that vendor and product ID is attached, and only then, its unconnected XInput slots are rechecked every 2 s. Microsoft's guidance is to recheck empty slots every few seconds at most ([Getting Started with XInput](https://learn.microsoft.com/en-us/windows/win32/xinput/getting-started-with-xinput)).
