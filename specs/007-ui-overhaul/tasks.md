...
- [ ] **P0-T4** Confirm "write our own D3D11 backend" decision (R2) is reflected in `contracts/rml-backend.h`. Acceptance: contract header matches the agreed RenderInterface surface; no upstream-backend fork referenced anywhere. Files: `specs/007-ui-overhaul/contracts/rml-backend.h` (verify only).
- [ ] **P0-T5** Confirm Win11 borderless recipe (R3) verified on both x64 and ARM64 in a throwaway probe, with runtime gating of Win11-only effects (Mica, rounded corners) so the app remains functional on Windows 10; record outcome in `research.md`. Acceptance: probe results table populated, no ARM64-specific deltas, `IsWindows11OrGreater()` fallback path exercised on Win10. Files: `specs/007-ui-overhaul/research.md`.
- [ ] **P0-T6** Confirm font decisions (R8): Inter (SIL OFL) for chrome/Skeuomorphic/Dark Modern; VT323 (SIL OFL) for Retro Terminal; both vendored under each theme's `fonts/` with `OFL.txt` alongside. Acceptance: research.md row R8 updated, font file sources captured. Files: `specs/007-ui-overhaul/research.md`.
