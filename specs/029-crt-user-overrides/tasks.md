...
- [ ] T002 [P] Add `#include "Core/JsonValue.h"` to `Casso/Config/MonitorCatalog.h`, which names `JsonValue` at `:105` and `JsonType` at `:109` while including neither. Create `UnitTest/UiTests/MonitorCatalogTests.cpp` and add its `<ClCompile>` entry to `UnitTest/UnitTest.vcxproj`, pinning the frozen identifiers `AppleMonitorII` and `AppleMonitorIIc`, that `Default()` is the first entry, and that `ByName` on an unknown name returns `Default()`. Assert presence of the two names, never an exact catalog size, so adding a third monitor is a decision rather than a red build.
...
- [ ] T006 Create `Casso/Config/CrtResolver.h` and `.cpp` implementing `ResolveCrt (preset, themeDefaults, overrides)` returning `CrtResolved`, per [contracts/resolver.md](contracts/resolver.md). Add both to `Casso/Casso.vcxproj` and the `.cpp` to `UnitTest/UnitTest.vcxproj`. Add `Touch`, `Clear (CrtField)` and `ClearAll` to `CrtOverrides`. Take the preset by value, never a mode index, and take no geometry.
...
- [ ] T027 [P] Correct stale prose left behind by the deletions: the layering description at `Casso/Config/CrtPresets.h:21-23` and the flag description at `Casso/Config/GlobalUserPrefs.h:118-120`. Note that `specs/007-ui-overhaul/contracts/global-user-prefs.schema.json` is two generations stale already and is a historical artifact, not an edit site.
...
- [ ] T029 Run `scripts\CheckStyle.ps1 -Against origin/master -Revision HEAD`. CI runs the checker in tree mode on every master push and this feature rewrites several function banner comments, which is exactly what it polices.
