...
- **An offline cycle reference, generated from the instruction tables.**
  `docs/cycle-reference.md` gives every opcode's mnemonic, addressing mode,
  length and base cycle count for the 6502 and the 65C02, side by side. It is
  built from the tables the emulator bills, regenerated by
  `scripts/UpdateCycleReference.ps1`, and a test fails when the document and
  the tables disagree.
